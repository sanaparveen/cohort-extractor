...
def make_engine():
    # engine = mssql_salchemyql_engine_from_url("mssql+pyodbc://sa:SanaParveen1234@localhost:1433?driver=ODBC+Driver+17+for+SQL+Server")
    engine = mssql_sqlalchemy_engine_from_url(os.environ["DATABASE_URL"])
...
class DonorsChooseOutcomes(Base):
    __tablename__ = "DonorsChooseOutcomes"

    id = Column(Integer, primary_key=True)

    is_exciting = Column(String)
    at_least_1_teacher_referred_donor = Column(String)
    fully_funded = Column(Boolean, default=False)
    at_least_1_green_donation = Column(Boolean, default=False)
    great_chat = Column(Boolean, default=False)
    three_or_more_non_teacher_referred_donors = Column(Boolean, default=False)
    OneNonTeacherReferredDonorGiving100Plus = Column(Boolean, default=False)
    DonationFromThoughtfulDonor = Column(Boolean, default=False)
    GreatMessageProportion = Column(Integer)
    TeacherReferredCount = Column(Integer)
    NonTeacherReferredCount = Column(Integer)
